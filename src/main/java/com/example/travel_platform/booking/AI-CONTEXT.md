...
숙소 예약, 지도 상세 화면, 예약 API 관련 백엔드 로직을 담당한다.
...
| BookingService.java | 예약 비즈니스 로직을 처리한다. |
...
- `/bookings/map-detail`, `/bookings/checkout`, `/bookings/complete` 화면과 `/api/bookings` 계열 응답을 함께 점검한다.

## 의존성

- 내부: `trip`, `src/main/resources/templates/pages`, `src/main/resources/static/js/map-detail.js`
- 외부: `Spring MVC`, `JPA/Hibernate`, `Kakao Map JS SDK`
