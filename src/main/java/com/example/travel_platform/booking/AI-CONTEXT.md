...
숙소 예약 SSR 화면, 예약 API, 지도 보조 API, 예약 완료 처리 흐름을 담당한다.
...
| `BookingService.java` | 예약 완료 기반 예약 생성, 예약 목록/상세/취소, 보조 API |
...
| `MapPlaceImageRepository.java` | 장소 이미지 캐시 대표 리포지토리 |

## 현재 구조 기준

- SSR 모델 이름은 `model` 단건 규칙을 사용한다.
...
- `Booking`은 `lodgingName`과 `roomName`을 분리해 저장한다.
- 지역은 자유 문자열 `location` 대신 `regionKey` 코드로 저장하고, 화면 표시용 지역명은 `Booking.getLocation()`과 `BookingResponse`에서 라벨로 제공한다.
- 예약 상태는 `BookingStatus` enum 문자열로 저장하고 취소 시각은 `cancelledAt`으로 기록한다.
...
- `booking-detail.mustache`는 `mypage`에서 사용하는 실제 예약 상세 화면이고, 취소 버튼은 `DELETE /api/bookings/{bookingId}`를 호출한다.

## 정규화 메모

- `Booking`은 `TripPlan`, `User`를 직접 참조하므로 예약 상세/마이페이지와 강하게 연결된다.
- `lodgingName`은 숙소 이름, `roomName`은 객실 이름으로 분리 저장한다.
- `location` 자유 문자열 저장은 제거하고 `regionKey` 코드 저장으로 전환했다.
- `status`, `cancelledAt`은 예약 이력 유지를 위한 최소 상태 구조다.
- `imageUrl`은 외부 연동 보조 데이터라 nullable 표시값으로 유지한다.
- `Lodging`, `MapPlaceImage`는 지도 보조 데이터 영역 구조라 별도 하위 폴더로 분리했다.
