...
- `CalendarResponse.EventDTO`는 `fromCalendarEvent(...)`, `DayNodeDTO`는 `createDayNode(...)` 이름 규칙을 사용한다.
- `getDayNodeList(...)`, `getDayNode(...)`는 이번 v4에서도 placeholder 상태를 유지한다.
...
- day node 구조는 현재 placeholder라 실제 구현/확장은 별도 승인 단위로 분리한다.
