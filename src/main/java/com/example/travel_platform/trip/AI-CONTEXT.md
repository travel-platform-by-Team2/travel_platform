...
- `TripService.getPlanList(...)`의 장소 수 집계는 `TripPlaceRepository.countByTripPlanIds(...)`로 배치 조회한다.
...
- `TripRepository`는 저장/삭제 영속화 책임만 두고, 조회는 `TripPlanQueryRepository`에 유지한다.
