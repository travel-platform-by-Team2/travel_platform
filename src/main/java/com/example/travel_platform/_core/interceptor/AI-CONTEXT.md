...
| 파일명 | 설명 |
| --- | --- |
| AdminInterceptor.java | `/admin`, `/admin/*` 요청에서 관리자 권한을 검사하고, 비활성 사용자나 동시 로그인으로 무효화된 세션을 로그인 페이지로 보낸다. |
| LoginInterceptor.java | 로그인 여부를 검사하고, 비활성 사용자나 동시 로그인으로 무효화된 세션을 API/MVC 요청별로 다르게 차단한다. |
...
- 비활성 일반 사용자와 동시 로그인으로 무효화된 old session 검사는 `user/UserSessionPolicyChecker.java`를 통해 공통 판정한다.
- 비활성 일반 사용자 세션은 일반 페이지 요청에서 alert 스크립트와 `/login-form` 이동, `/api/*` 요청에서는 세션 종료 후 403 상태로 처리한다.
- 동시 로그인으로 무효화된 세션은 일반 페이지 요청에서 `/login-form?forcedLogout=concurrent`로 redirect하고, `/api/*` 요청에서는 `401 + X-Session-Status=concurrent-login`으로 처리한다.
...
- 동시 로그인으로 old session이 되면 일반 요청은 `/login-form?forcedLogout=concurrent`, API 요청은 `401 + header`로 처리되는지 확인한다.
