...
| 파일명 | 설명 |
| --- | --- |
| AdminFilter.java | `/admin/*` 요청에서 `sessionUser.role == ADMIN` 여부를 검사한다. |
...
- 현재 `FilterConfig`는 `LoginFilter`를 `/boards/*`, `/calendar`, `/calendar/*`, `/api/calendar`, `/api/calendar/*`에, `AdminFilter`를 `/admin/*`에 등록한다.
...
- `GET /boards/{id}` 예외 통과, 일반 MVC redirect, `/api/calendar*` 401 반환, `/admin/*` 비관리자 차단이 각각 유지되는지 확인한다.
