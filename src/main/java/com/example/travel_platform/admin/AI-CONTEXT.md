...
| AdminUserQueryRepository.java | 관리자 화면에서 재사용하는 사용자 조회와 집계 JPQL 쿼리를 담당한다. |
...
- `AdminController`는 `renderDashboardPage(...)`, `renderUsersPage(...)`, `renderBoardsPage(...)` helper로 화면 진입 흐름을 분리한다.
- `/admin` 대시보드는 `model` 단건 루트 계약을 사용하고, `AdminService.getDashboardView()`가 실데이터 요약 카드, 차트 2개, 최근 유저/최근 게시글 3건을 조립한다.
- `/admin/users`는 `AdminService.getUserListView(...)`가 `model` 페이지 메타 DTO와 `models` 유저 목록 DTO를 분리해 반환한다.
- `/admin/boards`는 `AdminService.getBoardListView(...)`가 `model` 페이지 메타 DTO와 `models` 게시글 목록 DTO를 분리해 반환한다.
- 관리자 템플릿은 대시보드는 `model`, 유저/게시글 관리는 `model + models` 루트 계약을 사용한다.
...
- `AdminService.deleteBoardByAdmin(...)`는 `requireAdminSessionUser(...)`, `findDeleteTargetBoard(...)`, `deleteBoardLikeRelations(...)` 순서로 읽히게 정리됐다.
- `AdminUserQueryRepository`는 관리자 전용 사용자 목록/검색/최근 유저 조회를 JPQL로 담당하고, 단건 사용자 조회는 `UserRepository`를 사용한다.
...
- `AdminServiceDashboardTest`에서 대시보드, 유저 목록 view, 게시글 목록 view 조립 결과를 실제 데이터 기준으로 확인한다.
- `AdminServiceTest`에서 게시글 삭제의 `401/403/404/정상` 흐름과 게시글 목록 view 조회 경로를 mock 기반으로 확인한다.
- `AdminTemplateContractTest`에서 `admin-dashboard`, `admin-users`, `admin-boards`가 `model/models`와 `createdDate` 계약을 읽는지 확인한다.
